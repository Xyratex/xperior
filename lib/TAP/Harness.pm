...
sub _get_parser_args {
    ...
    if ( my $exec = $self->exec ) {
        #FIXME directly add parameters for test
        my $test_args =$self->test_args->{ $job->description };

        $args{exec}
          = ref $exec eq 'CODE'
          ? $exec->( $self, $test_prog, $test_args  )
          : [ @$exec, $test_prog, $test_args ];
